...
<<<<<<< HEAD
    ...
    /// <param name="r">row</param>
    /// <param name="h">heignt</param>
    /// <param name="itemObj">self reference</param>
    public void AddItem(int q, int r, int h, GameObject itemObj)
    {
        this.levelGrid[q, r, h].items.Add(itemObj);
    }

    /// <summary>
    /// AddProp is what individual prop objects use to inject themselves into the level's information
    /// </summary>
    /// <param name="q">column</param>
    /// <param name="r">row</param>
    /// <param name="h">heignt</param>
    /// <param name="propObj">self reference</param>
    public void AddProp(int q, int r, int h, GameObject propObj)
    {
        this.levelGrid[q, r, h].items.Add(propObj);
    }
    ...
    //If a cell has items, return their item prefabs
    //Works like GetEnemy in AIController, except it returns a list of item prefabs (stored on the items themselves)
    public List<GameObject> GetItemPrefabs(int q, int r, int h)
    {
        if (levelGrid[q, r, h] == null)
        {
            return null;
        }
        if (levelGrid[q, r, h].HasItems)
        {
            List<GameObject> minimapPrefabs = new List<GameObject>();
            foreach (GameObject item in levelGrid[q, r, h].items)
            {
                GameObject itemPrefab = item.GetComponent<SpecialItem>().minimapObject;
                minimapPrefabs.Add(itemPrefab);
            }
